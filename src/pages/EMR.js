...
//       shape: settings.shape
//     });
//   }, [settings.stimulus_type, settings.shape]);

//   // Initialize WebSocket for eye data (always receive but only save when stimulus active)
//   useEffect(() => {
//     webSocketRef.current = new WebSocket("ws://localhost:3001");
...
//           // Only start session when stimulus is active
//           if (!sessionStarted.current && isStimulusActive.current) {
//             setCurrentSession({
//               sessionStart: new Date(timestamp),
//               selectedEye: selectedEye?.toLowerCase() || "none",
//               stimulusType: settings.stimulus_type,
...
//             });
//             sessionStarted.current = true;
//             console.log("Started new session with stimulus active");
//           }

//           // Add raw data to buffer (always for plotting)
//           eyeDataBuffer.current.push({
//             angle_x,
//             angle_y,
//             timestamp
...
//           // Only save eye data to session when stimulus is active
...
//       }
//     };
//   }, [currentSession, selectedEye, settings.exam_mode, settings.stimulus_type, settings.shape]);

//   // Process and plot eye data (always for visualization)
//   const processAndPlotEyeData = () => {
//     if (eyeDataBuffer.current.length === 0) return;
...
//     // Update cumulative data for plotting (always)
//     cumulativeData.current = {
//       ...cumulativeData.current,
//       labels: [...cumulativeData.current.labels, newLabel],
//       eyeX: [...cumulativeData.current.eyeX, newEyeX],
...
//   // Periodic session save (only when stimulus is active)
//   useEffect(() => {
//     const interval = setInterval(() => {
...
//   // Start new session for stimulus (only when stimulus conditions are met)
//   const startNewStimulusSession = () => {
//     if (settings.shape && settings.stimulus_type && selectedEye && settings.exam_mode) {
//       const newSession = {
//         sessionStart: new Date(),
...
//   // Handle settings changes - only start session when stimulus is configured
//   useEffect(() => {
//     if (settings.shape && settings.stimulus_type && selectedEye && settings.exam_mode) {
//       // Conditions for stimulus are met, start/update session
//       if (!currentSession || currentSession.stimulusType !== settings.stimulus_type) {
//         // Save previous session if exists and start new one
//         if (currentSession && sessionDataPoints.current.length > 0) {
//           saveCurrentSession().then(() => {
//             resetPlottingState();
//             startNewStimulusSession();
...
//       }
//     } else {
//       // Stimulus conditions not met, stop any active session
//       if (currentSession && sessionDataPoints.current.length > 0) {
//         saveCurrentSession();
//       }
//       setCurrentSession(null);
...
//   // Process stimulus data (only when stimulus is active)
//   useEffect(() => {
//     if (!plottingEnabled || !currentSession) return;
...
//         // Save data point (only when stimulus is active)
//         if (isStimulusActive.current) {
//           const dataPoint = {
//             timestamp: new Date(now),
//             relativeTime: (now - currentSession.sessionStart.getTime()) / 1000,
//             eyeX: lastValidEyeData.current.eyeX,
...
//   // Initialize WebSocket for eye data (only once)
//   useEffect(() => {
//     if (webSocketConnected.current) {
//       return; // WebSocket already connected, don't create another one
//     }
...
//           // Only start session when stimulus is active and not paused
//           if (!sessionStarted.current && isStimulusActive.current && !isPaused) {
//             setCurrentSession({
//               sessionStart: new Date(timestamp),
//               selectedEye: selectedEye?.toLowerCase() || "none",
//               stimulusType: settings.stimulus_type,
...
//           // Add raw data to buffer (always for plotting, but only process if not paused)
//           eyeDataBuffer.current.push({
//             angle_x,
//             angle_y,
//             timestamp
...
//           // Only process and plot if not paused
//           if (!isPaused) {
//             processAndPlotEyeData();

//             // Only save eye data to session when stimulus is active and not paused
//             if (isStimulusActive.current && currentSession) {
//               const latest = eyeDataBuffer.current[eyeDataBuffer.current.length - 1];
//               const dataPoint = {
//                 timestamp: new Date(timestamp),
...
//       } catch (err) {
//         console.error("Error parsing WebSocket message:", err);
//         // Don't show toast for parsing errors to avoid spam
//       }
//     };

//     webSocketRef.current.onerror = (error) => {
//       console.error("WebSocket error:", error);
//       // Only show toast for connection errors, not for message parsing errors
//       if (!webSocketConnected.current) {
...
//       }
//     };
//   }, []); // Empty dependency array - WebSocket is created only once

//   // Process and plot eye data (always for visualization, but only if not paused)
//   const processAndPlotEyeData = () => {
...
//   // Periodic session save (only when stimulus is active and not paused)
//   useEffect(() => {
//     const interval = setInterval(() => {
//       if (isStimulusActive.current && currentSession && sessionDataPoints.current.length > 0 && !isPaused) {
//         saveCurrentSession(true);
//       }
//     }, SESSION_SAVE_INTERVAL);
...
//   // Process stimulus data (only when stimulus is active and not paused)
//   const startStimulusPlotting = () => {
//     if (!plottingEnabled || !currentSession || isPaused) return;
...
//         // Save data point (only when stimulus is active and not paused)
//         if (isStimulusActive.current && !isPaused) {
//           const dataPoint = {
//             timestamp: new Date(now),
//             relativeTime: (now - currentSession.sessionStart.getTime()) / 1000,
//             eyeX: lastValidEyeData.current.eyeX,
...
//   // Check plotting conditions for stimulus
//   useEffect(() => {
//     const shouldPlotStimulus = centerFocus && settings.stimulus_type && settings.shape && csvDataLoaded.current && lastEyeTimestamp.current && selectedEye && !isPaused;
//     console.log("Stimulus plotting conditions:", {
//       centerFocus,
//       stimulus_type: settings.stimulus_type,
//       shape: settings.shape,
...
//       stopStimulusPlotting();
//       if (isPaused) {
//         // When pausing, keep the current index so we can resume from same position
//         console.log("Paused at stimulus index:", currentDataIndex.current);
//       } else {
//         // When stopping completely (not just pausing), reset the state
//         resetPlottingState();
//       }
//     } else {
//       stimulusStartTime.current = Date.now();
//       console.log("Stimulus plotting enabled, starting from index:", currentDataIndex.current);
//     }
//   }, [centerFocus, settings.stimulus_type, settings.shape, csvDataLoaded.current, lastEyeTimestamp.current, selectedEye, isPaused]);
...
//     if (!isPaused) {
//       toast.info("Plotting paused");
//     } else {
//       toast.info("Plotting resumed");
//     }
//   };
...
//       // Always update last valid value so when we resume, no jump occurs
//       if (angle_x != null && angle_y != null) {
//         lastValidEyeData.current = { eyeX: angle_x, eyeY: angle_y };
//       }

//       // STOP EVERYTHING RELATED TO EYE DATA WHEN PAUSED
//       if (isPaused) return;
...
//       processAndPlotEyeData(); // Now safe — only called when not paused

//       // Auto-start session
//       if (!sessionStarted.current && isStimulusActive.current && !stimulusDataComplete.current) {
//         setCurrentSession({
//           sessionStart: new Date(timestamp),
//           selectedEye: selectedEye?.toLowerCase() || "none",
//           stimulusType: settings.stimulus_type,
//           stimulusShape: settings.shape,
//           examMode: settings.exam_mode,
//           settingsHistory: [{ stimulusType: settings.stimulus_type, stimulusShape: settings.shape, examMode: settings.exam_mode, timestamp: new Date() }],
//           dataPoints: []
//         });
//         sessionStarted.current = true;
//       }

//       // Record eye data to session only when stimulus is running
//       if (isStimulusActive.current && currentSession && !stimulusDataComplete.current) {
//         sessionDataPoints.current.push({
//           timestamp: new Date(timestamp),
//           relativeTime: (timestamp - currentSession.sessionStart.getTime()) / 1000,
//           eyeX: angle_x,
//           eyeY: angle_y,
...
//   const processAndPlotEyeData = () => {
//     // Don't process ANY data if paused
//     if (eyeDataBuffer.current.length === 0 || isPaused) return;
...
//     // Update cumulative data for plotting (only if not paused)
//     cumulativeData.current = {
//       ...cumulativeData.current,
//       labels: [...cumulativeData.current.labels, newLabel],
//       eyeX: [...cumulativeData.current.eyeX, newEyeX],
...
//     // Update chart data (only if not paused)
//     setChartData(prev => ({
//       ...prev,
//       labels: cumulativeData.current.labels,
//       datasets: [
...
//   useEffect(() => {
//     const interval = setInterval(() => {
//       if (isStimulusActive.current && currentSession && sessionDataPoints.current.length > 0 && !isPaused && !stimulusDataComplete.current) {
//         saveCurrentSession(true);
//       }
//     }, SESSION_SAVE_INTERVAL);
...
//   // Process stimulus data (plot only once through the CSV data) - UPDATED
//   const startStimulusPlotting = () => {
...
//           if (isStimulusActive.current && !isPaused) {
//             const dataPoint = {
//               timestamp: new Date(now + (i * (EYE_DATA_INTERVAL / pointsToProcess))),
//               relativeTime: (now - currentSession.sessionStart.getTime()) / 1000,
//               eyeX: lastValidEyeData.current.eyeX,
//               eyeY: lastValidEyeData.current.eyeY,
//               stimX: newStimX,
...
//         // Final check after processing - if we just processed the last points
//         if (currentDataIndex.current >= plottingData.stimX.length) {
//           console.log("Final points processed - completing stimulus plotting");
//           stimulusDataComplete.current = true;
//           stopStimulusPlotting();
//           setPlottingEnabled(false);
//           return;
...
//     }
//   }, [centerFocus, settings.stimulus_type, settings.shape, csvDataLoaded.current, lastEyeTimestamp.current, selectedEye, isPaused, stimulusDataComplete.current]);
...
//         xDataLength: limitedXData.length,
//         yDataLength: limitedYData.length,
//         estimatedTime: (limitedXData.length / 4) * 0.005 // points / points_per_interval * interval
//       });
//     } catch (error) {
//       console.error("Error fetching or parsing CSV files:", error);
//       toast.error("Error loading stimulus data from CSV files.");
  ...
  const togglePause = () => {
    setIsPaused((prev) => {
      ...
      if (newPaused) {
        ...
        // RESUMING: Save previous session if needed and start new session
        if (shouldSaveOnPause.current) {
          saveCurrentSession(false).then(() => {
            // After saving, start new session if stimulus is still active
            if (isStimulusActive.current && !stimulusDataComplete.current) {
              startNewStimulusSession();
            }
          });
          shouldSaveOnPause.current = false;
        }
...
//                     <div className="basis-[40%] p-4 rounded-md bg-white shadow-lg">
//                       <div className="w-full flex flex-col gap-6">
//                         {/* Horizontal Chart - Conditionally Rendered */}
//                         {!isPaused ? (
//                           <div className="h-[400px] bg-white p-4 rounded-md shadow-sm">
//                             <h4 className="text-center font-semibold text-black mb-2">
//                               Horizontal Eye Angle and Stimulus (X)
...
//                         {/* Vertical Chart - Conditionally Rendered */}
//                         {!isPaused ? (
//                           <div className="h-[400px] bg-white p-4 rounded-md shadow-sm">
//                             <h4 className="text-center font-semibold text-black mb-2">
//                               Vertical Eye Angle and Stimulus (Y)
